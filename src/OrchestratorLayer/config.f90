module config_base
  ...
  ! =REFACTOR=
  ! type, extends(NoahmpIO_type) :: NOAHLSM_OFFLINE
  ...
  ! end type NOAHLSM_OFFLINE

  ! =REFACTOR= these opts have been assimulated into noahmpvar
...
! #endif
  ...
  subroutine read_wrf_hydro_offline_namelist(f_in)
    implicit none

    integer, intent(in) :: f_in
    integer  :: ierr
    integer  :: finemesh, finemesh_factor, forc_typ, snow_assim
    ...
    rewind(f_in)
    read(f_in, NML=WRF_HYDRO_OFFLINE, iostat=ierr)
    if (ierr /= 0) then
  ...
  end subroutine read_wrf_hydro_offline_namelist
  ...
  subroutine init_noah_lsm_and_wrf_hydro()
    ...
    ! namelist /WRF_HYDRO_OFFLINE/ &
    !      finemesh,finemesh_factor,forc_typ, snow_assim
    print *, "REFACTOR TODO: NEED TO FIGURE OUT NOAHLSM_OFFLINE FOR NCDEP_WCOSS"
    ! noah_lsm%nsoil                   = -999
    ! ! dtbl                             = -999
...
! #ifndef NCEP_WCOSS
    ...
    print *, "REFACTOR TODO: NAMELISTS DIFFS NEED TO BE CONFIRMED"

    open(newunit=f_unit, file="namelist.hrldas", form="FORMATTED")
    call read_crocus_namelist(crocus_opts, f_unit)
    call read_wrf_hydro_offline_namelist(f_unit)
    close(f_unit)
! #else
!     open(newunit=f_unit, form="FORMATTED")
!     read(f_unit, NML=NOAHLSM_OFFLINE, iostat=ierr)

!     if (ierr /= 0) then
!        write(*,'(/," ***** ERROR: Problem reading namelist NOAHLSM_OFFLINE",/)')
!             rewind(f_unit)
!             read(f_unit, NOAHLSM_OFFLINE)
!        stop "FATAL ERROR: Problem reading namelist NOAHLSM_OFFLINE"
!     endif

!     read(f_unit, NML=WRF_HYDRO_OFFLINE, iostat=ierr)
!     if (ierr /= 0) then
!        write(*,'(/," ***** ERROR: Problem reading namelist WRF_HYDRO_OFFLINE",/)')
!        call hydro_stop (" FATAL ERROR: Problem reading namelist WRF_HYDRO_OFFLINE")
!     endif
...
! #endif
    ...
    ! noah_lsm%IOPT_DVEG = dynamic_veg_option
    ! noah_lsm%IOPT_CRS = canopy_stomatal_resistance_option
    ! noah_lsm%IOPT_BTR = btr_option
    ! noah_lsm%IOPT_RUNOFF = runoff_option ! =REFACTOR= this not in refactor namelist
    ! noah_lsm%IOPT_SFC = surface_drag_option
    ! noah_lsm%IOPT_FRZ = supercooled_water_option
    ! noah_lsm%IOPT_INF = frozen_soil_option
    ! noah_lsm%IOPT_RAD = radiative_transfer_option
    ! noah_lsm%IOPT_ALB = snow_albedo_option
    ! noah_lsm%IOPT_SNF = pcp_partition_option
    ! noah_lsm%IOPT_TBOT = tbot_option
    ! noah_lsm%IOPT_STC = temp_time_scheme_option
    ! noah_lsm%IOPT_GLA = glacier_option
    ! noah_lsm%IOPT_RSF = surface_resistance_option
    ...
    ! noah_lsm%IOPT_SOIL = soil_data_option
    ! noah_lsm%IOPT_PEDO = pedotransfer_option
    ! noah_lsm%IOPT_CROP = crop_option
    ! noah_lsm%IOPT_IMPERV = imperv_option
    ...
    ! allocate(noah_lsm%soil_thick_input(nsoil))
    ! noah_lsm%soil_thick_input = soil_thick_input(1:nsoil)
